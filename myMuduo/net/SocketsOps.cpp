...
// 把sockaddr_in6*重新解释为通用的sockaddr*，用于调用那些只接受struct sockaddr类型参数的POSIX socket API，
//并没有把IPv4转换为IPv6,	完整的命名空间是myMuduo::net::sockets
...
// 为什么IPv4不需要返回非const类型的sockaddr_cast函数暂时还不明白
//struct sockaddr* sockets::sockaddr_cast(struct sockeaddr_in* addr) {
...
int sockets::accept(int sockfd, struct sockaddr_in6* addr) {
	...
	if (connfd < 0) {
		...
		case EPROTO:	// ???
		case EPERM:
		case EMFILE:	// per-process limit of open descriptor ???
			// expected errors
			errno = savedErrno;
			break;
		case EBADF:
...
void sockets::close(int sockfd) {
	if (0 > ::close(sockfd))	LOG_SYSERR << "sockets::close";
}

void sockets::shutdownWrite(int sockfd) {
	if (0 > ::shutdown(sockfd, SHUT_WR))	LOG_SYSERR << "sockets::shutdownWrite";
}
...
int sockets::getSocketError(int sockfd) {
	...
	if (0 > ::getsockopt(sockfd, SOL_SOCKET, SO_ERROR, &optval, &optlen))	return errno;
	else return optval;
}
