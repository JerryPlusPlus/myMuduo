	...
	namespace detail {
		...
		struct Comp
		{
			...
			bool operator()(const Transition& lhs, const Transition& rhs)const {
				if (compareGmt)	return lhs.gmttime < rhs.gmttime;
				else return lhs.Localtime < rhs.Localtime;
			}
...
using myMuduo::detail::Transition;
using myMuduo::detail::Localtime;
using myMuduo::TimeZone;
using std::vector;
using std::string;
	...
	namespace detail {
		...
		public:
			...
			int32_t readInt32() {
				...
				if (sizeof(int32_t) != nr) throw std::logic_error("bad int32_t data");
				return be32toh(x);	// be32toh 是一个字节序转换函数，缩写：big endian 32 to host
			}
			uint8_t readUInt8() {
				int8_t x = 0;
				ssize_t nr = ::fread(&x, 1, sizeof(uint8_t), fp_);
				if (sizeof(int8_t) != nr) throw std::logic_error("bad uint8_t data");
				return x;
			}
		private:
			FILE* fp_;
		...
		bool readTimeZoneFile(const char* zonefile, struct TimeZone::Data* data) {
			...
			if (f.vaild()) {
				try {
					...
					for (int i = 0; i < timecnt; ++i)	trans.push_back(f.readInt32());
		...
		const struct Localtime* findLocaltime(const TimeZone::Data& data, Transition sentry, Comp comp) {
			const struct Localtime* local = nullptr;
			if (data.transitions.empty() || comp(sentry, data.transitions.front())) {
				...
				if (data.transitions.end() != transI) {
					if (!comp.equal(sentry, *transI)) {
						assert(data.transitions.begin() != transI);
						--transI;
					}
...
{
	if (!detail::readTimeZoneFile(zonefile, data_.get()))	data_.reset();
}
