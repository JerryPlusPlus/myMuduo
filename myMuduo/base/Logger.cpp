...
namespace myMuduo {
	...
	Logger::LogLevel initLogLevel() {
		if (::getenv("MUDUO_LOG_TRACE"))	return Logger::LogLevel::TRACE;
		else if (::getenv("MUDUO_LOG_DEBUG"))	return Logger::LogLevel::DEBUG;
	...
	struct T {
		T(const char* str, unsigned len) :str_(str), len_(len) { assert(len_  == strlen(str)); }
		const char* str_;
		const unsigned len_;
	};
	...
	void defaultOutput(const char* msg, int len) {
		...
		// FIXME check n
		(void)n;	//作用是 显式“使用”变量 n，从而压制“未使用变量”告警。
	}
	...
	basename_(file) 
{
	...
	if (savedErrno != 0)	stream_ << strerror_tl(savedErrno) << " (errno" << savedErrno << ") ";
}
...
void Logger::Impl::formatTime() {
	...
	if (t_lastSecond != seconds) {
		...
		struct tm tm_time;
		if (g_logTimeZone.valid())	tm_time = g_logTimeZone.toLocalTime(seconds);
		else ::gmtime_r(&seconds, &tm_time);	// FIXME TimeZone::fromUtcTime
...
Logger::~Logger() {
	...
	if (LogLevel::FATAL == impl_.level_) {
		g_flush();
		abort();
	}
}
